...
项目根目录在RotorPy之上增加了一条六旋翼专用验证链路：19类健康/单电机故障
独立重复仿真 → 数据质量审查 → 独立来源分组滑窗 → Fold内WGAN-GP增广 →
分组4折分类与安全指标。当前代码不再包含轴承数据集适配分支。

正式实验请严格按 [`RUNNING_ORDER.md`](RUNNING_ORDER.md) 执行。核心规则包括：

- 默认4组配对独立仿真；同一`run_id`的19类共享扰动条件并作为一个不可拆分组；
- 故障CSV只取故障后稳定段，Normal取匹配时段，避免轨迹阶段成为类别线索；
- 控制器使用健康名义矩阵做有界分配，不读取故障真值补偿；
- `effectiveness_*`、故障标签和预测机体扳手只作真值/审计，正式特征白名单不读取；
- 分类器强制使用`StratifiedGroupKFold`，没有`groups.npy`就停止；
- GAN和分类器使用各自的折内scaler，测试折不参与拟合；
- 除准确率/F1外，还报告故障漏检、正常误报、电机定位和严重度准确率；
- 非零命令—推力一致性正则是可选实验项，默认关闭，不宣称完整物理GAN。

飞机本体参数见
[`docs/HEXAROTOR_PHYSICAL_CHARACTERISTICS.md`](docs/HEXAROTOR_PHYSICAL_CHARACTERISTICS.md)，
文件结构见 [`PROJECT_STRUCTURE.md`](PROJECT_STRUCTURE.md)。
